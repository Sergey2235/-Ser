...
class LoginForm:
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(card, text="🚗 АвтоТранс", font=(FONT_FAMILY, 20, "bold"), fg=HEADER_BG, bg=CARD_BG).pack(pady=(0, PAD * 2))
        tk.Label(card, text="Вход в систему", font=(FONT_FAMILY, 11), fg=TEXT_MUTED, bg=CARD_BG).pack(pady=(0, PAD))
        tk.Label(card, text="Логин:", font=(FONT_FAMILY, 10), bg=CARD_BG).pack(anchor="w")
        self.entry_login = tk.Entry(card, width=32, font=(FONT_FAMILY, 11), relief="solid", bd=1)
        self.entry_login.pack(pady=(2, PAD))
        tk.Label(card, text="Пароль:", font=(FONT_FAMILY, 10), bg=CARD_BG).pack(anchor="w")
        self.entry_password = tk.Entry(card, show="*", width=32, font=(FONT_FAMILY, 11), relief="solid", bd=1)
        self.entry_password.pack(pady=(2, PAD))
        tk.Button(card, text="Войти", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=24, pady=8, cursor="hand2", command=self._login).pack(pady=PAD)
        hint = tk.Frame(self.root, bg=BG_COLOR)
        hint.place(relx=0.5, rely=0.92, anchor="center")
        tk.Label(hint, text="Менеджер: login1 / pass1  ·  Оператор: login4 / pass4  ·  Механик: login2 / pass2  ·  Заказчик: login12 / pass12", font=(FONT_FAMILY, 8), fg=TEXT_MUTED, bg=BG_COLOR).pack()
    ...
    def _login(self) -> None:
        ...
        if user:
            ...
            messagebox.showerror("Ошибка входа", "Неверный логин или пароль. Проверьте данные и попробуйте снова.")
...
class MainForm:
    def __init__(self, root: tk.Tk, user: User) -> None:
        ...
        header.pack(fill="x")
        tk.Label(header, text=f"🚗 АвтоТранс  ·  {user.fio}", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 12)).pack(side="left", padx=PAD * 2, pady=14)
        tk.Button(header, text="Выход", bg=DANGER, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=16, pady=6, cursor="hand2", command=self._logout).pack(side="right", padx=PAD * 2, pady=10)

        content = tk.Frame(self.root, bg=BG_COLOR, padx=PAD * 3, pady=PAD * 3)
        content.pack(fill="both", expand=True)
        tk.Label(content, text=f"Добро пожаловать, {user.fio}!", font=(FONT_FAMILY, 16), bg=BG_COLOR).pack(pady=(PAD, PAD * 2))
        tk.Label(content, text=f"Роль: {user.user_type}", font=(FONT_FAMILY, 11), fg=TEXT_MUTED, bg=BG_COLOR).pack(pady=(0, PAD))

        if user.user_type in ("Оператор", "Менеджер", "Менеджер по качеству"):
            ...
            tk.Label(stats_frame, text=f"📋 Заявок всего: {total_r}   ·   В работе: {active_r}", font=(FONT_FAMILY, 11), bg=CARD_BG).pack(anchor="w")

        card = tk.Frame(content, bg=CARD_BG, padx=PAD * 2, pady=PAD * 2)
        card.pack(pady=PAD)
        if self.user.user_type in ("Оператор", "Менеджер"):
            tk.Button(card, text="📋 Заявки", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_requests).pack(pady=6)
            tk.Button(card, text="📊 Статистика", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_statistics).pack(pady=6)
        elif self.user.user_type == "Автомеханик":
            tk.Button(card, text="📋 Мои заявки", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_requests).pack(pady=6)
        elif self.user.user_type == "Заказчик":
            tk.Button(card, text="📋 Мои заявки", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_requests).pack(pady=6)
        elif self.user.user_type == "Менеджер по качеству":
            tk.Button(card, text="📋 Заявки", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_requests).pack(pady=6)
            tk.Button(card, text="📊 Статистика", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 11), relief="flat", padx=20, pady=12, cursor="hand2", command=self._open_statistics).pack(pady=6)
...
class RequestsForm:
    def __init__(self, root: tk.Toplevel, user: User) -> None:
        ...
        header.pack(fill="x")
        tk.Label(header, text="📋 Заявки", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 12)).pack(side="left", padx=PAD * 2, pady=12)
        if self.user.user_type in ("Оператор", "Менеджер", "Менеджер по качеству"):
            tk.Button(header, text="➕ Добавить заявку", bg=SUCCESS, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=12, pady=6, cursor="hand2", command=self._open_new_request).pack(side="right", padx=6, pady=10)
        tk.Button(header, text="Назад", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 10), relief="flat", cursor="hand2", command=self.root.destroy).pack(side="right", padx=PAD, pady=10)

        STATUSES = ("Все", "Новая заявка", "В процессе ремонта", "Готова к выдаче")
        ...
        self.combo_status = ttk.Combobox(filter_frame, values=STATUSES, width=18, state="readonly")
        ...
        tk.Button(filter_frame, text="Найти", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=12, pady=4, cursor="hand2", command=self._apply_search).pack(side="left", padx=2)
        tk.Button(filter_frame, text="Сбросить", font=(FONT_FAMILY, 10), relief="flat", cursor="hand2", command=self._reset_search).pack(side="left", padx=2)
    ...
    def _load_requests(self, search_text: str | None = None) -> None:
        for item in self.tree.get_children():
            self.tree.delete(item)
        status_filter = self.combo_status.get().strip() if hasattr(self, "combo_status") else "Все"
        if status_filter == "":
            status_filter = "Все"
        search_text = search_text if search_text is not None else (self.entry_search.get().strip() or None)
...
class NewRequestForm:
    ...
    def __init__(self, root: tk.Toplevel, on_success: callable) -> None:
        ...
        self.combo_car_type = ttk.Combobox(card, values=list(self.CAR_TYPES), width=38, state="readonly")
        self.combo_car_type.grid(row=1, column=1, pady=6, padx=PAD)
        self.combo_car_type.set(self.CAR_TYPES[0])
        ...
        tk.Button(btn_frame, text="Сохранить", bg=SUCCESS, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=20, pady=8, cursor="hand2", command=self._save).pack(side="left", padx=8)
        tk.Button(btn_frame, text="Отмена", font=(FONT_FAMILY, 10), relief="flat", cursor="hand2", command=self.root.destroy).pack(side="left", padx=8)

    def _save(self) -> None:
        if not self.client_names:
            messagebox.showwarning("Ошибка", "В системе нет заказчиков. Добавьте пользователя с ролью «Заказчик».")
            return
        idx = self.combo_client.current()
        if idx < 0:
            messagebox.showwarning("Ошибка ввода", "Выберите клиента (заказчика).")
            return
        ...
        if new_id is None:
            messagebox.showerror("Ошибка", "Не удалось создать заявку. Проверьте данные и попробуйте снова.")
            return
...
class RequestDetailsForm:
    ...
    def __init__(self, root: tk.Toplevel, user: User, request_id: int, on_close=None) -> None:
        self.root = root
        self.user = user
        ...
        header = tk.Frame(self.root, bg=HEADER_BG, height=48)
        header.pack(fill="x")
        tk.Label(header, text=f"📋 Заявка #{request_id}", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 12)).pack(side="left", padx=PAD * 2, pady=12)
        tk.Button(header, text="Назад", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 10), relief="flat", cursor="hand2", command=self._on_close).pack(side="right", padx=PAD * 2, pady=10)
        ...
        raw = get_request_status_and_master(request_id)
        self.current_status = raw[0] if raw else "Новая заявка"
        self.current_master_id = raw[1] if raw else None
        can_edit = user.user_type in ("Оператор", "Менеджер", "Менеджер по качеству") or (
            user.user_type == "Автомеханик" and self.current_master_id == user.user_id
        )
        can_edit_mech = user.user_type in ("Оператор", "Менеджер", "Менеджер по качеству")
        can_comment = can_edit or (user.user_type == "Автомеханик" and self.current_master_id == user.user_id)
        can_delete = user.user_type in ("Оператор", "Менеджер")
        ...
        if row:
            ...
            if repair_parts and repair_parts.strip():
                tk.Label(card, text=f"Запчасти: {repair_parts.strip()}", font=(FONT_FAMILY, 10), bg=CARD_BG, wraplength=720, justify="left").pack(anchor="w", pady=4)
        ...
        if can_edit or can_edit_mech or can_comment or can_delete:
            ...
            if can_edit:
                ...
                self.combo_status = ttk.Combobox(row2, values=list(self.STATUSES), width=20, state="readonly")
                self.combo_status.set(self.current_status)
                self.combo_status.pack(side="left", padx=4)
                tk.Button(row2, text="Сохранить статус", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 9), relief="flat", padx=10, pady=4, cursor="hand2", command=self._save_status).pack(side="left", padx=8)
            if can_edit_mech:
                mechanics = get_users_by_type("Автомеханик")
                ...
                tk.Button(row2, text="Назначить", bg=BTN_COLOR, fg="white", font=(FONT_FAMILY, 9), relief="flat", padx=10, pady=4, cursor="hand2", command=self._assign_master).pack(side="left", padx=8)
            if can_delete:
                tk.Button(row2, text="Удалить заявку", bg=DANGER, fg="white", font=(FONT_FAMILY, 9), relief="flat", padx=10, pady=4, cursor="hand2", command=self._delete_request).pack(side="left", padx=8)
        ...
        if can_comment:
            ...
            tk.Button(comm_row, text="Добавить", bg=SUCCESS, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=12, pady=4, cursor="hand2", command=self._add_comment).pack(side="left")
        ...
        if QR_CODE_AVAILABLE:
            tk.Button(btn_frame, text="📱 QR-код для отзыва", bg=SUCCESS, fg="white", font=(FONT_FAMILY, 10), relief="flat", padx=16, pady=8, cursor="hand2", command=self._show_qr).pack(side="left", padx=6)
        else:
            tk.Label(btn_frame, text="QR-код недоступен (установите qrcode и Pillow)", font=(FONT_FAMILY, 9), fg=TEXT_MUTED, bg=BG_COLOR).pack(side="left", padx=6)
    ...
    def _fill_comments(self) -> None:
        ...
        if comments:
            for msg, author, date in comments:
                tk.Label(self.comments_card, text=f"{author} ({date}): {msg}", font=(FONT_FAMILY, 9), bg="#e2e8f0", anchor="w").pack(fill="x", padx=6, pady=4)
        else:
            tk.Label(self.comments_card, text="Комментариев нет.", font=(FONT_FAMILY, 10), fg=TEXT_MUTED, bg=CARD_BG).pack(anchor="w", padx=6)

    def _save_status(self) -> None:
        new_status = self.combo_status.get().strip()
        if not new_status or new_status not in self.STATUSES:
            return
...
class StatisticsForm:
    def __init__(self, root: tk.Toplevel) -> None:
        ...
        header = tk.Frame(self.root, bg=HEADER_BG, height=48)
        header.pack(fill="x")
        tk.Label(header, text="📊 Статистика", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 12)).pack(side="left", padx=PAD * 2, pady=12)
        tk.Button(header, text="Назад", fg=HEADER_FG, bg=HEADER_BG, font=(FONT_FAMILY, 10), relief="flat", cursor="hand2", command=self.root.destroy).pack(side="right", padx=PAD * 2, pady=10)

        content = tk.Frame(self.root, bg=BG_COLOR, padx=PAD * 2, pady=PAD * 2)
        content.pack(fill="both", expand=True)
        tk.Label(content, text="Статистика работы сервиса", font=(FONT_FAMILY, 14, "bold"), bg=BG_COLOR).pack(pady=(0, PAD))
        ...
        if avg_time > 0:
            ...
            tk.Label(card, text="Среднее время ремонта: нет завершённых заявок", font=(FONT_FAMILY, 11), fg=TEXT_MUTED, bg=CARD_BG).pack(anchor="w", pady=4)

        tk.Label(content, text="Топ-5 типов неисправностей", font=(FONT_FAMILY, 11, "bold"), bg=BG_COLOR).pack(anchor="w", pady=(PAD, 4))
